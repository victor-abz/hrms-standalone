...
def get_leave_approval_details(employee: str) -> dict:
    ...
    if leave_approver and leave_approver not in [
...
def get_department_approvers(department: str, parentfield: str) -> list[str]:
    ...
    department_details = frappe.db.get_value(
        "Department", department, ["lft", "rgt"], as_dict=True
    )
...
def get_employee_advance_balance(employee: str) -> list[dict]:
    ...
    advances = (
        ...
        .select(
            ...
            Advance.purpose,
            Advance.paid_amount,
            (Advance.paid_amount - (Advance.claimed_amount + Advance.return_amount)).as_(
                "balance_amount"
            ),
...
def get_currency_symbols() -> dict:
    ...
    currencies = (frappe.qb.from_(Currency).select(Currency.name, Currency.symbol)).run(
        as_dict=True
    )
