frappe.provide('hrms.timesheet');

hrms.timesheet.timer = function (frm, row, timestamp = 0) {
  ...
  hrms.timesheet.control_timer(frm, dialog, row, timestamp);
  dialog.show();
};

hrms.timesheet.control_timer = function (frm, dialog, row, timestamp = 0) {
