...
from basic.controllers.status_updater import StatusUpdater
...
class TransactionBase(StatusUpdater):
        ...
        for reference_doctype, ref_dn_list in ref_doc.items():
            ...
            for reference_name in ref_dn_list:
                ...
                if not prevdoc_values:
                    frappe.throw(
                        _("Invalid reference {0} {1}").format(reference_doctype, reference_name)
                    )
    ...
    def validate_rate_with_reference_doc(self, ref_details):
        ...
        for ref_dt, ref_dn_field, ref_link_field in ref_details:
            reference_names = [
                d.get(ref_link_field) for d in self.get("items") if d.get(ref_link_field)
            ]
    ...
    def reset_default_field_value(
        self, default_field: str, child_table: str, child_table_field: str
    ):
...
def validate_uom_is_integer(doc, uom_field, qty_fields, child_dt=None):
    ...
    integer_uoms = list(
        filter(
            lambda uom: frappe.db.get_value("UOM", uom, "must_be_whole_number", cache=True)
            or None,
            distinct_uoms,
        )
    )
