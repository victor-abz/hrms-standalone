...
frappe.ui.form.on('Timesheet', {
  ...
  set_route_options_for_new_task: (frm) => {
    ...
    if (task_field) {
      task_field.get_route_options_for_new_doc = (row) => ({
        project: row.doc.project,
      });
    }
  },
...
frappe.ui.form.on('Timesheet Detail', {
  ...
  activity_type: function (frm, cdt, cdn) {
    ...
    frappe.call({
      method: 'hrms.projects.doctype.timesheet.timesheet.get_activity_cost',
      args: {
...
var calculate_end_time = function (frm, cdt, cdn) {
  ...
  if (child.hours) {
    ...
    frappe.model
      .set_value(cdt, cdn, 'to_time', d.format(frappe.defaultDatetimeFormat))
      .then(() => {
