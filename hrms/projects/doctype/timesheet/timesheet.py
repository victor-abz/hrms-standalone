...
class Timesheet(Document):
    ...
    def calculate_total_amounts(self):
        ...
        for d in self.get("time_logs"):
            ...
            if d.is_billable:
                ...
                self.base_total_billed_amount += (
                    flt(d.base_billing_amount) if d.sales_invoice else 0.0
                )
    ...
    def update_billing_hours(self, args):
        if args.is_billable:
            if flt(args.billing_hours) == 0.0:
                ...
                frappe.msgprint(
                    _("Warning - Row {0}: Billing Hours are more than Actual Hours").format(
                        args.idx
                    ),
                    indicator="orange",
                    alert=True,
    ...
    def validate_dates(self):
        for data in self.time_logs:
            if (
                data.from_time
                and data.to_time
                and time_diff_in_hours(data.to_time, data.from_time) < 0
            ):
    ...
    def validate_overlap(self, data):
        ...
        self.validate_overlap_for(
            "employee", data, self.employee, settings.ignore_employee_time_overlap
        )
    ...
    def update_cost(self):
        for data in self.time_logs:
            if data.activity_type or data.is_billable:
                ...
                if rate:
                    data.billing_rate = (
                        flt(rate.get("billing_rate"))
                        if flt(data.billing_rate) == 0
                        else data.billing_rate
                    )
                    data.costing_rate = (
                        flt(rate.get("costing_rate"))
                        if flt(data.costing_rate) == 0
                        else data.costing_rate
                    )
