...
from hrms.projects.doctype.task.task import CircularReferenceError
...
class TestTask(unittest.TestCase):
    def test_circular_reference(self):
        ...
        task2 = create_task(
            "_Test Task 2", add_days(nowdate(), 11), add_days(nowdate(), 15), task1.name
        )
        task3 = create_task(
            "_Test Task 3", add_days(nowdate(), 11), add_days(nowdate(), 15), task2.name
        )
    ...
    def test_reschedule_dependent_task(self):
        ...
        self.assertEqual(
            frappe.db.get_value("Task", task2.name, "exp_start_date"),
            getdate(add_days(nowdate(), 21)),
        )
        self.assertEqual(
            frappe.db.get_value("Task", task2.name, "exp_end_date"),
            getdate(add_days(nowdate(), 25)),
        )

        self.assertEqual(
            frappe.db.get_value("Task", task3.name, "exp_start_date"),
            getdate(add_days(nowdate(), 26)),
        )
        self.assertEqual(
            frappe.db.get_value("Task", task3.name, "exp_end_date"),
    ...
    def test_overdue(self):
        ...
        from hrms.projects.doctype.task.task import set_tasks_as_overdue
