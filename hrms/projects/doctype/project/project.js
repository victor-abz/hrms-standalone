...
frappe.ui.form.on('Project', {
  ...
  onload: function (frm) {
    ...
    frm.set_query('user', 'users', function () {
      return {
        query: 'hrms.projects.doctype.project.project.get_users_for_project',
      };
    });
      ...
      if (frappe.model.can_read('Task')) {
        ...
        frm.add_custom_button(
          ...
          () => {
            frappe
              .call(
                'hrms.projects.doctype.project.project.create_kanban_board_if_not_exists',
                {
                  project: frm.doc.name,
                }
              )
  ...
  update_total_purchase_cost: function (frm) {
    frappe.call({
      method:
        'hrms.projects.doctype.project.project.recalculate_project_total_purchase_cost',
  ...
  create_duplicate: function (frm) {
    return new Promise((resolve) => {
      frappe.prompt('Project Name', (data) => {
        frappe
          .xcall(
            'hrms.projects.doctype.project.project.create_duplicate_project',
            {
              prev_doc: frm.doc,
  ...
  set_status: function (frm, status) {
    frappe.confirm(
      ...
      () => {
        frappe
          .xcall('hrms.projects.doctype.project.project.set_project_status', {
            project: frm.doc.name,
            status: status,
          })
          .then(() => {
