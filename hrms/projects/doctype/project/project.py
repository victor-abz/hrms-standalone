...
from basic.controllers.queries import get_filters_cond
from basic.controllers.website_list_for_contact import get_customers_suppliers
...
class Project(Document):
    ...
    if TYPE_CHECKING:
        ...
        from hrms.projects.doctype.project_user.project_user import ProjectUser
        ...
        if not total:
            ...
            if self.percent_complete_method == "Task Weight" and total > 0:
                ...
                for row in weighted_progress:
                    pct_complete += row["progress"] * frappe.utils.safe_div(
                        row["task_weight"], weight_sum
                    )
...
def get_list_context(context=None):
    from basic.controllers.website_list_for_contact import get_list_context
...
def weekly_reminder():
    ...
    for project in projects:
        ...
        if allow_to_make_project_update(
            project.name, project.get("weekly_time_to_send"), "Weekly"
        ):
            send_project_update_email_to_users(project.name)
...
def update_project_sales_billing():
    sales_update_frequency = frappe.db.get_single_value(
        "Selling Settings", "sales_update_frequency"
    )
