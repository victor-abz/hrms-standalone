...
def get_columns(filters, mode_of_payments):
    ...
    for mode in mode_of_payments:
        columns.append(
            {"label": _(mode), "fieldname": mode, "fieldtype": "Currency", "width": 160}
        )
...
def get_data(filters, mode_of_payments):
    ...
    if data:
        ...
        data.append(
            {"branch": "<b>Total Net Pay</b>", mode_of_payments[0]: total_row.get("total")}
        )

        currency = hrms.get_company_currency(filters.company)
