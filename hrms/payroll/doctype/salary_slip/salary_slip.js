...
frappe.ui.form.on('Salary Slip', {
  setup: function (frm) {
    ...
    frm.fields_dict['timesheets'].grid.get_field('time_sheet').get_query =
      function () {
        return {
          filters: {
  ...
  refresh: function (frm) {
    ...
    frm.fields_dict['earnings'].grid.set_column_disp(
      salary_detail_fields,
      false
    );
    frm.fields_dict['deductions'].grid.set_column_disp(
      salary_detail_fields,
      false
    );
    frm.trigger('set_dynamic_labels');
  ...
  set_payment_days_description: function (frm) {
    ...
    frappe
      .call('hrms.payroll.utils.get_payroll_settings_for_payment_days')
      .then((r) => {
        const {
          payroll_based_on,
        ...
        const message = `
          ...
          )}: ${consider_unmarked_attendance_as.bold()}
