...
class SalarySlip(TransactionBase):
    ...
    def payroll_period(self):
        if not hasattr(self, "__payroll_period"):
            self.__payroll_period = get_payroll_period(
    ...
    def actual_start_date(self):
        if not hasattr(self, "__actual_start_date"):
            ...
            if self.joining_date and getdate(self.start_date) < self.joining_date <= getdate(
                self.end_date
            ):
    ...
    def check_existing(self):
        if not self.salary_slip_based_on_timesheet:
            ...
            if ret_exist:
                frappe.throw(
                    _("Salary Slip of employee {0} already created for this period").format(
                        self.employee
                    )
                )
        else:
    ...
    def set_time_sheet(self):
        if self.salary_slip_based_on_timesheet:
            ...
            for data in timesheets:
                self.append(
                    "timesheets", {"time_sheet": data.name, "working_hours": data.total_hours}
                )
    ...
    def get_payment_days(self, include_holidays_in_total_working_days):
        ...
        if self.relieving_date:
            ...
            if self.relieving_date < getdate(self.start_date) and employee_status != "Left":
                frappe.throw(
                    _("Employee relieved on {0} must be set as 'Left'").format(self.relieving_date)
                )
    ...
    def calculate_lwp_or_ppl_based_on_leave_application(
        ...
        for d in working_days_list:
            ...
            if cint(leave.half_day) and (
                leave.half_day_date == d or leave.from_date == leave.to_date
            ):
    ...
    def calculate_lwp_ppl_and_absent_days_based_on_attendance(
        ...
        for d in attendance_details:
            ...
            if d.status == "Half Day":
                ...
                if (
                    d.leave_type in leave_type_map.keys()
                    and leave_type_map[d.leave_type]["is_ppl"]
                ):
                    equivalent_lwp *= (
                        fraction_of_daily_salary_per_leave
    ...
    def compute_current_and_future_taxable_earnings(self):
        ...
        self.current_taxable_earnings = self.get_taxable_earnings(
            self.tax_slab.allow_tax_exemption
        )
        self.future_structured_taxable_earnings = (
            self.current_taxable_earnings.taxable_earnings * (ceil(self.remaining_sub_periods) - 1)
        )
            ...
            if deduction.exempted_from_income_tax:
                if deduction.amount_based_on_formula:
                    for sub_period in range(1, ceil(self.remaining_sub_periods)):
                        future_period_exempted_amount += self.get_amount_from_formula(
                            deduction, sub_period
                        )
                else:
    ...
    def get_amount_from_formula(self, struct_row, sub_period=1):
        ...
        local_data.update(
            {"start_date": start_date, "end_date": end_date, "posting_date": posting_date}
        )
    ...
    def calculate_component_amounts(self, component_type):
        if not getattr(self, "_salary_structure_doc", None):
            self._salary_structure_doc = frappe.get_cached_doc(
    ...
    def add_structure_components(self, component_type):
        ...
        for struct_row in self._salary_structure_doc.get(component_type):
            if (
                self.salary_slip_based_on_timesheet
                and struct_row.salary_component == timesheet_component
            ):
                continue
            ...
            if struct_row.statistical_component:
                ...
                if struct_row.depends_on_payment_days:
                    ...
                    self.data[struct_row.abbr] = flt(
                        payment_days_amount, struct_row.precision("amount")
                    )
                ...
                if (
                    ...
                    or (
                        not remove_if_zero_valued
                        and amount is not None
                        and not self.data[struct_row.abbr]
                    )
                ):
    ...
    def get_component_abbr_map(self):
        def _fetch_component_values():
            return {
                component_abbr: 0
                for component_abbr in frappe.get_all(
            ...
            if getdate(payroll_period.end_date) <= getdate(self.end_date):
                ...
                if last_benefits:
                    for last_benefit in last_benefits:
                        ...
                        self.update_component_row(
                            frappe._dict(last_benefit.struct_row), amount, "earnings"
                        )
    ...
    def add_tax_components(self):
        ...
        if not tax_components:
            tax_components = [
                d.salary_component
                for d in self.get("deductions")
                if d.variable_based_on_taxable_salary
            ]
    ...
    def update_component_row(
        ...
        for d in self.get(component_type):
            ...
            if (
                not d.additional_salary and (not additional_salary or additional_salary.overwrite)
            ) or (additional_salary and additional_salary.name == d.additional_salary):
                component_row = d
                break
    ...
    def get_income_tax_slabs(self):
        ...
        if getdate(income_tax_slab_doc.effective_from) > getdate(self.payroll_period.start_date):
            frappe.throw(
                ...
                ).format(self.payroll_period.start_date)
            )
    ...
    def get_tax_paid_in_period(self, start_date, end_date, tax_component):
        ...
        tax_deducted_till_date = self.get_opening_for(
    ...
    def get_future_recurring_period(
        ...
        if not to_date:
            to_date = frappe.db.get_value(
                "Additional Salary", additional_salary, "to_date", cache=True
            )
    ...
    def set_totals(self):
        ...
        if self.salary_slip_based_on_timesheet == 1:
            ...
            self.net_pay = (
                ...
                - flt(self.get("total_loan_repayment"))
            )
...
def calculate_tax_by_tax_slab(
    ...
    for slab in tax_slab.slabs:
        ...
        if not slab.to_amount and annual_taxable_earning >= slab.from_amount:
            tax_amount += (
                (annual_taxable_earning - slab.from_amount + 1) * slab.percent_deduction * 0.01
            )
            continue
        ...
        if annual_taxable_earning >= slab.from_amount and annual_taxable_earning < slab.to_amount:
            ...
            annual_taxable_earning >= slab.from_amount and annual_taxable_earning >= slab.to_amount
        ):
