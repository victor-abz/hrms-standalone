...
from hrms.setup.doctype.employee.employee import InactiveEmployeeStatusError
...
class TestSalarySlip(FrappeTestCase):
    ...
    @change_settings(
        "Payroll Settings",
        {"payroll_based_on": "Attendance", "daily_wages_fraction_for_half_day": 0.75},
    )
    ...
    def test_payment_days_based_on_leave_application(self):
        ...
        make_leave_application(
            emp_id, first_sunday, add_days(first_sunday, 3), "Leave Without Pay"
        )
    ...
    def test_component_amount_dependent_on_another_payment_days_based_component(self):
        ...
        employee = make_employee(
            "test_payment_days_based_component@salary.com", company="_Test Company"
        )
        ...
        for component in ss.earnings:
            if component.salary_component == "HRA - Payment Days":
                payment_days_based_comp_amount = flt(
                    component.amount, component.precision("amount")
                )
                break
        ...
        expected_amount = flt(
            (flt(ss.gross_pay) - payment_days_based_comp_amount) * 0.12, precision
        )
    ...
    def test_loan_repayment_salary_slip(self):
        ...
        applicant = make_employee(
            "test_loan_repayment_salary_slip@salary.com", company="_Test Company"
        )
        ...
        payroll_period = create_payroll_period(
            name="_Test Payroll Period", company="_Test Company"
        )
        ...
        ss = make_employee_salary_slip(
            applicant, "Monthly", "Test Loan Repayment Salary Structure"
        )
        ss.submit()
        ...
        self.assertEqual(
            ss.net_pay,
            (flt(ss.gross_pay) - (flt(ss.total_deduction) + flt(ss.total_loan_repayment))),
        )
    ...
    def test_multi_currency_salary_slip(self):
        ...
        applicant = make_employee(
            "test_multi_currency_salary_slip@salary.com", company="_Test Company"
        )
        frappe.db.sql(
    ...
    def test_tax_for_payroll_period(self):
        ...
        data["benefit-1"] = create_benefit_claim(
            employee, payroll_period, 15000, "Medical Allowance"
        )
    ...
    def test_default_amount(self):
        ...
        employee = make_employee(
            "test_tax_for_mid_joinee@salary.com", date_of_joining=joining_date
        )
        ...
        self.assertEqual(
            ss.earnings[2].amount,
            flt(ss.earnings[0].amount * 0.5, ss.earnings[0].precision("amount")),
        )
    ...
    def test_tax_for_recurring_additional_salary(self):
        ...
        create_recurring_additional_salary(
            employee, "Performance Bonus", 20000, start_date, end_date
        )
    ...
    def test_statistical_component_based_on_payment_days(self):
        ...
        self.assertEqual(
            amount, flt((1000 * ss.payment_days / ss.total_working_days) * 0.5, precision)
        )
    ...
    def test_salary_slip_generation_against_opening_entries_in_ssa(self):
        ...
        tax_slab = create_tax_slab(
            payroll_period, effective_date="2022-04-01", allow_tax_exemption=True
        )
        ...
        salary_slip = make_salary_slip(
            ...
            employee=employee_doc.name,
            posting_date=getdate("2023-10-01"),
        )
    ...
    def test_income_tax_breakup_fields(self):
        ...
        payroll_period = frappe.get_all(
            "Payroll Period", filters={"company": "_Test Company"}, limit=1
        )
        ...
        create_additional_salary_for_non_taxable_component(
            emp, payroll_period, company="_Test Company"
        )
    ...
    def test_lwp_calculation_based_on_relieving_date(self):
        ...
        frappe.db.set_value(
            "Employee", emp_id, {"relieving_date": relieving_date, "status": "Left"}
        )
    ...
    def test_component_default_amount_against_statistical_component(self):
        ...
        # Create Salary Slip
        salary_slip = make_salary_slip(
            salary_structure_doc.name, employee=emp, posting_date=nowdate()
        )
...
def set_salary_component_account(sal_comp, company_list=None):
    ...
    if not sal_comp.get("accounts"):
        for d in company_list:
            ...
            sal_comp.append(
                "accounts",
                {"company": d, "account": create_account(account_name, d, parent_account)},
            )
            sal_comp.save()
