...
class PayrollEntry(Document):
    ...
    def validate_existing_salary_slips(self):
        ...
        if len(emp_with_sal_slip):
            frappe.throw(
                _("Salary Slip already exists for {0}").format(comma_and(emp_with_sal_slip))
            )
    ...
    def get_sal_slip_list(self, ss_status, as_dict=False):
        ...
        ss_list = (
            ...
            .where(
                ...
                & (
                    Coalesce(ss.salary_slip_based_on_timesheet, 0)
                        ...
                        self.update_accounting_dimensions(
                            {
                                ...
                                "debit_in_account_currency": flt(
                                    amount_against_cost_center, precision
                                ),
                                "exchange_rate": flt(exchange_rate),
                                "reference_type": self.doctype,
                                "reference_name": self.name,
    ...
    def update_salary_slip_status(self, submitted_salary_slips, jv_name=None):
        ...
        (
            ...
            .where(
                SalarySlip.name.isin([salary_slip.name for salary_slip in submitted_salary_slips])
            )
        ).run()
    ...
    def get_employees_with_unmarked_attendance(self) -> list[dict] | None:
        ...
        for emp in self.employees:
            details = next(
                (record for record in employee_details if record.name == emp.employee), None
            )
            if not details:
                continue
    ...
    def get_payroll_dates_for_employee(self, employee_details: dict) -> tuple[str, str]:
        ...
        end_date = self.end_date
        if employee_details.relieving_date and employee_details.relieving_date < getdate(
            self.end_date
        ):
    ...
    query = (
        ...
        .where(
            ...
            & (
                ...
                == filters.payroll_payable_account
            )
