...
frappe.ui.form.on('Salary Structure', {
  ...
  // 	hrms.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function(account){
  ...
  set_earning_deduction_component: function (frm) {
    ...
    frm.set_query('salary_component', 'earnings', function () {
      return {
        ...
        query:
          'hrms.payroll.doctype.salary_structure.salary_structure.get_salary_component',
      };
    });
  ...
  refresh: function (frm) {
    ...
    if (frm.doc.docstatus === 1) {
      frm
        .add_custom_button(__('Bulk Assign Structure'), () => {
  ...
  assign_to_employees: function (frm) {
    ...
    d.fields_dict.grade.df.onchange = function () {
      ...
      if (grade) {
        frappe.db
          .get_value('Employee Grade', grade, 'default_base_pay')
          .then(({ message }) => {
  ...
  preview_salary_slip: function (frm) {
    frappe.call({
      method:
        'hrms.payroll.doctype.salary_structure.salary_structure.get_employees',
      args: {
