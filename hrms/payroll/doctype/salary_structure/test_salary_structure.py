...
class TestSalaryStructure(FrappeTestCase):
    ...
    def test_amount_totals(self):
        ...
        if not salary_slip:
            ...
            self.assertEqual(
                salary_slip.get("net_pay"), 78000 - salary_slip.get("total_deduction")
            )

    def test_whitespaces_in_formula_conditions_fields(self):
        salary_structure = make_salary_structure(
            "Salary Structure Sample", "Monthly", dont_submit=True
        )
    ...
    def test_employee_grade_defaults(self):
        ...
        employee = make_employee(
            "test_employee_grade@salary.com", company="_Test Company", grade="Lead"
        )
    ...
    def test_multi_currency_salary_structure(self):
        ...
        sal_struct = make_salary_structure(
            "Salary Structure Multi Currency", "Monthly", currency="USD"
        )
...
def create_salary_structure_assignment(
    ...
    if not allow_duplicate and frappe.db.exists(
        ...
        frappe.db.sql(
            """delete from `tabSalary Structure Assignment` where employee=%s""", (employee)
        )
    ...
    if not income_tax_slab:
        income_tax_slab = create_tax_slab(
            payroll_period, allow_tax_exemption=True, currency=currency
        )
