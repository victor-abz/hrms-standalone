...
from basic.setup.doctype.employee.employee import get_employee_email
...
class ExitInterview(Document):
    ...
    def validate_duplicate_interview(self):
        doc = frappe.db.exists(
            "Exit Interview",
            {"employee": self.employee, "name": ("!=", self.name), "docstatus": ("!=", 2)},
        )
        if doc:
            frappe.throw(
...
def validate_questionnaire_settings():
    ...
    if (
        not settings.exit_questionnaire_web_form
        or not settings.exit_questionnaire_notification_template
    ):
        frappe.throw(
