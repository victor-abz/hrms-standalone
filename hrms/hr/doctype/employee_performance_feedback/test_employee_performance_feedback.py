...
class TestEmployeePerformanceFeedback(FrappeTestCase):
    def setUp(self):
        ...
        self.employee = make_employee(
            "employee@example.com", company=company, designation="Engineer"
        )
        self.reviewer1 = make_employee(
            "reviewer1@example.com", company=company, designation="Engineer"
        )
        self.reviewer2 = make_employee(
            "reviewer2@example.com", company=company, designation="Engineer"
        )

        cycle = create_appraisal_cycle(designation="Engineer")
        cycle.create_appraisals()

        self.appraisal = frappe.db.get_all("Appraisal", filters={"appraisal_cycle": cycle.name})[
            0
        ].name
