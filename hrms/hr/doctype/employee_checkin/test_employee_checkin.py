...
class TestEmployeeCheckin(FrappeTestCase):
    ...
    def test_add_log_based_on_employee_field(self):
        ...
        employee_checkin = add_log_based_on_employee_field(
            "3344", time_now, "mumbai_first_floor", "IN"
        )
    ...
    def test_fetch_night_shift_for_assignment_without_end_date(self):
        ...
        for log in [log_in, log_out]:
            ...
            self.assertEqual(
                log.shift_actual_end, datetime.combine(next_day, get_time("02:00:00"))
            )
...
def make_n_checkins(employee, n, hours_to_reverse=1):
    logs = [
        make_checkin(employee, now_datetime() - timedelta(hours=hours_to_reverse, minutes=n + 1))
    ]
    for i in range(n - 1):
        logs.append(
            make_checkin(
                employee, now_datetime() - timedelta(hours=hours_to_reverse, minutes=n - i)
            )
        )
    return logs
