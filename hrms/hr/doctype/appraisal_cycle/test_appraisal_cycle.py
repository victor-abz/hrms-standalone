...
class TestAppraisalCycle(FrappeTestCase):
    def setUp(self):
        ...
        self.employee1 = make_employee(
            "employee1@example.com", company=company, designation="Engineer"
        )
    ...
    def test_create_appraisals(self):
        ...
        for i in range(2):
            ...
            self.assertEqual(
                appraisal.appraisal_kra[i].kra, self.template.goals[i].key_result_area
            )
            self.assertEqual(
                appraisal.appraisal_kra[i].per_weightage, self.template.goals[i].per_weightage
            )

            # check if rating criteria is set
            self.assertEqual(
                appraisal.self_ratings[i].criteria, self.template.rating_criteria[i].criteria
            )
            self.assertEqual(
