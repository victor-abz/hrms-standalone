...
class TestShiftAssignment(FrappeTestCase):
    ...
    def test_overlap_for_midnight_shifts(self):
        ...
        for i, pair in enumerate(overlapping_shifts):
            s1 = setup_shift_type(
                shift_type=f"Shift 1-{i}", start_time=pair[0][0], end_time=pair[0][1]
            )
            s2 = setup_shift_type(
                shift_type=f"Shift 2-{i}", start_time=pair[1][0], end_time=pair[1][1]
            )
        ...
        shift_type = setup_shift_type(
            shift_type="Shift 1", start_time="20:00:00", end_time="01:00:00"
        )
        make_shift_assignment(shift_type.name, employee, date)

        # no overlap
        shift_type = setup_shift_type(
            shift_type="Shift 2", start_time="15:00:00", end_time="20:00:00"
        )
    ...
    def test_calendar_for_night_shift(self):
        ...
        shift_type = setup_shift_type(
            shift_type="Shift 1", start_time="08:00:00", end_time="02:00:00"
        )
        date = getdate()
    ...
    def test_consecutive_day_and_night_shifts(self):
        ...
        employee = make_employee(
            "test_default_shift_assignment@example.com", company="_Test Company"
        )
        today = getdate()
