...
class TestExpenseClaim(FrappeTestCase):
    ...
    def test_expense_claim_status_as_payment_from_journal_entry(self):
        ...
        claim = make_expense_claim(
            payable_account, 1000, 0, "_Test Company", "Travel Expenses - _TC"
        )
    ...
    def test_repost(self):
        ...
        gl_entries = frappe.db.get_all(
            "GL Entry",
            filters={"account": expense_claim.payable_account, "voucher_no": expense_claim.name},
        )
...
def create_payment_reconciliation(company, employee, payable_account):
    ...
    pr.from_invoice_date = (
        pr.to_invoice_date
    ) = pr.from_payment_date = pr.to_payment_date = nowdate()
    return pr
