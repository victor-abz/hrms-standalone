...
class LeaveApplication(Document, PWANotificationsMixin):
    ...
    def create_or_update_attendance(self, attendance_name, date):
        ...
        if attendance_name:
            ...
            doc.db_set(
                {"status": status, "leave_type": self.leave_type, "leave_application": self.name}
            )
        else:
    ...
    def validate_block_days(self):
        ...
        if block_dates and self.status == "Approved":
            frappe.throw(
                _("You are not authorized to approve leaves on Block Dates"), LeaveDayBlockedError
            )
    ...
    def notify_employee(self):
        ...
        if not template:
            frappe.msgprint(
                _("Please set default template for Leave Status Notification in HR Settings.")
            )
            return
...
def get_remaining_leaves(
    ...
    if cf_expiry and allocation.unused_leaves:
        ...
        leave_balance = (flt(allocation.new_leaves_allocated) + flt(new_leaves_taken)) + flt(
            cf_leaves
        )
