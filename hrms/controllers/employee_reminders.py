...
def send_holidays_reminder_in_advance(employee, holidays):
    ...
    email_header = (
        _("Holidays this Month.") if frequency == "Monthly" else _("Holidays this Week.")
    )
    frappe.sendmail(
        sender=sender_email,
        ...
        args=dict(
            ...
            ).format(employee_doc.get("first_name")),
    ...
    for company, birthday_persons in employees_born_today.items():
        ...
        if len(birthday_persons) > 1:
            ...
            for person in birthday_persons:
                person_email = (
                    person["user_id"] or person["personal_email"] or person["company_email"]
                )
...
def get_birthday_reminder_text_and_message(birthday_persons):
    ...
    message += "<br>"
    message += _("Everyone, let’s congratulate {0} on their birthday.").format(
        birthday_person_text
    )
