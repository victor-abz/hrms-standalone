...
from hrms.utils.transaction_base import delete_events
...
class Employee(NestedSet):
            ...
            if reports_to:
                link_to_employees = [
                    frappe.utils.get_link_to_form(
                        "Employee", employee.name, label=employee.employee_name
                    )
    ...
    def validate_duplicate_user_id(self):
        ...
        if employee:
            throw(
                _("User {0} is already assigned to Employee {1}").format(
                    self.user_id, employee[0][0]
                ),
...
def get_holiday_list_for_employee(employee, raise_exception=True):
    ...
    if not holiday_list and raise_exception:
        frappe.throw(
            _("Please set a default Holiday List for Employee {0} or Company {1}").format(
                employee, company
            )
        )
...
def get_children(doctype, parent=None, company=None, is_root=False, is_tree=False):
    ...
    for employee in employees:
        is_expandable = frappe.get_all(
            doctype, filters=[["reports_to", "=", employee.get("value")]]
        )
