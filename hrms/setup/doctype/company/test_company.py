...
from hrms.setup.doctype.company.company import get_default_company_address
...
class TestCompany(unittest.TestCase):
            ...
            for template in templates:
                try:
                    ...
                    for account_type in account_types:
                        ...
                        error_message = _("No Account matched these filters: {}").format(
                            json.dumps(filters)
                        )
        ...
        for company in records:
            ...
            if parent_company:
                parent_lft, parent_rgt = frappe.db.get_value(
                    "Company", parent_company, ["lft", "rgt"]
                )
            else:
                # root
    ...
    def test_demo_data(self):
        from hrms.setup.demo import clear_demo_data, setup_demo_data
        ...
        for transaction in frappe.get_hooks("demo_transaction_doctypes"):
            self.assertTrue(
                frappe.db.exists(frappe.unscrub(transaction), {"company": company_name})
            )
