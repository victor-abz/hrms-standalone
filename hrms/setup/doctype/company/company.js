...
frappe.ui.form.on('Company', {
  ...
  date_of_commencement: function (frm) {
    if (frm.doc.date_of_commencement < frm.doc.date_of_incorporation) {
      frappe.throw(
        __('Date of Commencement should be greater than Date of Incorporation')
      );
    }
  ...
  refresh: function (frm) {
    ...
    if (!frm.is_new()) {
      ...
      if (
        frappe.perm.has_perm('Purchase Taxes and Charges Template', 0, 'read')
      ) {
        frm.add_custom_button(
    ...
    frappe.verify_password(function () {
      var d = frappe.prompt(
        ...
        function (data) {
          ...
          frappe.call({
            method:
              'hrms.setup.doctype.company.company.create_transaction_deletion_request',
            args: {
