...
class Company(NestedSet):
    ...
    def validate_perpetual_inventory(self):
        if not self.get("__islocal"):
            if cint(self.enable_perpetual_inventory) == 1 and not self.default_inventory_account:
                frappe.msgprint(
                    _("Set default inventory account for perpetual inventory"),
                    alert=True,
                    indicator="orange",
                )
    ...
    def set_default_accounts(self):
        ...
        if not self.disposal_account:
            disposal_acct = frappe.db.get_value(
                "Account",
                {
                    "account_name": _("Gain/Loss on Asset Disposal"),
                    "company": self.name,
                    "is_group": 0,
                },
            )
    ...
    def on_trash(self):
        ...
        warehouses = frappe.db.sql_list(
            "select name from tabWarehouse where company=%s", self.name
        )
        if warehouses:
            frappe.db.sql(
        ...
        if boms:
            ...
            for dt in ("BOM Operation", "BOM Item", "BOM Scrap Item", "BOM Explosion Item"):
                frappe.db.sql(
                    "delete from `tab%s` where parent in (%s)"
                    "" % (dt, ", ".join(["%s"] * len(boms))),
                    tuple(boms),
                )
        ...
        frappe.db.sql(
            "delete from `tabSales Taxes and Charges Template` where company=%s", self.name
        )
        frappe.db.sql(
            "delete from `tabPurchase Taxes and Charges Template` where company=%s", self.name
        )
        ...
        if not frappe.db.get_value("GL Entry", {"company": self.name}):
            frappe.db.sql(
                "delete from `tabProcess Deferred Accounting` where company=%s", self.name
            )
...
def update_company_monthly_sales(company):
    ...
    frappe.db.set_value(
        "Company", company, "sales_monthly_history", json.dumps(month_to_value_dict)
    )
