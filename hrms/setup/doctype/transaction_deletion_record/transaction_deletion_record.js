...
frappe.ui.form.on('Transaction Deletion Record', {
  onload: function (frm) {
    if (frm.doc.docstatus == 0) {
      ...
      frappe.call({
        method:
          'hrms.setup.doctype.transaction_deletion_record.transaction_deletion_record.get_doctypes_to_be_ignored',
        callback: function (r) {
          doctypes_to_be_ignored_array = r.message;
          populate_doctypes_to_be_ignored(doctypes_to_be_ignored_array, frm);
          frm.fields_dict['doctypes_to_be_ignored'].grid.set_column_disp(
            'no_of_docs',
            false
          );
