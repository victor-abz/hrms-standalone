...
class TransactionDeletionRecord(Document):
    ...
    if TYPE_CHECKING:
        ...
        from hrms.setup.doctype.transaction_deletion_record_item.transaction_deletion_record_item import (
    ...
    def delete_version_log(self, doctype, company_fieldname):
        ...
        names = (
            qb.from_(dt)
            .select(dt.name)
            .where(dt[company_fieldname] == self.company)
            .run(as_list=1)
        )
    ...
    def unlink_attachments(self, doctype, reference_doc_names):
        files = frappe.get_all(
            ...
            filters={
                ...
                "attached_to_name": ["in", reference_doc_names],
            },
        )
        ...
        for batch in create_batch(file_names, self.batch_size):
            qb.update(file).set(file.attached_to_doctype, None).set(
                file.attached_to_name, None
            ).where(file.name.isin(batch)).run()
