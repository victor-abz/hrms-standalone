...
class EmailDigest(Document):
    ...
    if TYPE_CHECKING:
        ...
        from hrms.setup.doctype.email_digest_recipient.email_digest_recipient import (
    ...
    def get_msg_html(self):
        ...
        from hrms.setup.doctype.email_digest.quotes import get_random_quote
    ...
    def get_notifications(self):
        ...
        notifications = sorted(
            notifications.get("open_count_doctype", {}).items(), key=lambda a: a[1]
        )

        notifications = [
            {"key": n[0], "value": n[1], "link": get_url_to_list(n[0])}
                ...
                if card:
                    ...
                    if card.last_value:
                        ...
                        card.last_value = self.fmt_money(
                            card.last_value,
                            False if key in ("bank_balance", "credit_balance") else True,
                        )
    ...
    def get_period_amounts(self, accounts, fieldname):
        ...
        for account in accounts:
            balance += get_incomes_expenses_for_period(
                account, self.future_from_date, self.future_to_date
            )
            past_balance += get_incomes_expenses_for_period(
                account, self.past_from_date, self.past_to_date
            )
            count += get_count_for_period(
                account, fieldname, self.future_from_date, self.future_to_date
            )
    ...
    def get_type_balance(self, fieldname, account_type, root_type=None):
        if root_type:
            ...
            accounts = [
                ...
                for d in frappe.db.get_all(
                    "Account",
                    filters={"account_type": account_type, "company": self.company, "is_group": 0},
                )
            ]
        ...
        for account in accounts:
            ...
            count += get_count_on(account, fieldname, date=self.future_to_date)
            prev_balance += get_balance_on(
                account, date=self.past_to_date, in_account_currency=False
            )
    ...
    def get_pending_purchase_orders(self):
        return self.get_summary_of_pending(
            "Purchase Order", "pending_purchase_orders", "per_received"
        )
...
def get_count_for_period(account, fieldname, from_date, to_date):
    ...
    if from_date == fy_start_date:
        ...
        last_year_closing_count = get_count_on(
            account, fieldname, fy_start_date - timedelta(days=1)
        )
